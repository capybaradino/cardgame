...
def card_play_get(sid):
    ...
    for board in viewdata.p1board:
        if (board is not None):
            i = board.locnum
            p1banmen[i] = card_play_util.card_createcardhtml(
                board, "p1board", i)
    p2banmen = []
    i = 0
    while (i < 6):
    ...
    for board in viewdata.p2board:
        if (board is not None):
            i = board.locnum
            p2banmen[i] = card_play_util.card_createcardhtml(
                board, "p2board", i)
    # プレイヤー
    ...
    if (viewdata.turnstate == "p1turn"):
        turnendbutton = '<input type=button value="Turn end" onclick="system_turnend()">'
    else:
        turnendbutton = ""
