...
class TestAPITension(unittest.TestCase):
    ...
    def test_api_tension_with_tension_skill_wiz_unit_dont_exist(self):
        ...
        self.assertEqual(result, ({"error": "unit don't exists in card2"}, 403))

    def test_api_tension_with_tension_skill_wiz_antieffect(self):
        self.playview.p1tension = 3
        self.playview.p2board[0] = Mock()
        self.playview.p2board[0].refresh = Mock()
        self.playview.p2board[0].refresh.return_value = None
        self.playview.p2board[0].status = ["antieffect"]
        result = api_tension.api_tension(
            self.sid, self.playview, "card1", "rightboard_0"
        )
        self.assertEqual(result, ({"error": "card2 has antieffect"}, 403))
